...
- **Chat Box Detail (Stitch Style):** 
  ...
  - In the bottom left corner, there are dropdowns for **Model (Auto/Fast/Pro)**, **Tournament (World Cup/AFC/AFCON/UEFA)**, **Mode (Live Standings/Random)**, and **Style (Commentator Style, Coach Style, Football Analyst Style)**.
...
- **External API Caching (Critical for Rate Limiting):** Currently, the project runs entirely on local data (`football_data_standings.json`) and does not make external requests. However, to handle the 10 requests/minute limit of the Football-Data.org free tier, the backend must implement a cache layer (e.g., in-memory or Redis) when external data integration is implemented in the future. Static data like standings and fixtures will be cached for a set duration (e.g., 2 hours for standings, 24 hours for team list/fixtures) to prevent direct hits on the external API for every user request.
